...
vec2 scene(vec3 x)
{
    ...
    for(int i=0; i<5; ++i)
    {
        float zi = length(x-c.yxy
            ...
            /*-.2*rand(float(i)*c.xx)*/+.1*iScale-.52;
        //d = min(d, zi);
...
//        d = softmin(d, length(x)-rand(float(i)*c.xx), 8.);
    ...
    //voronoi
    vec2 dis = 2.*vec2((.1+.05*iScale)*valuenoise(x.xy-2.-1.*iTime),(.1+.05*iScale)*valuenoise(x.xy-5.-1.*iTime));
    vec3 va = .5*vor(2.*x.xy-dis-iNBeats);
    ind = vec3(va.yz,0.);
    d = softmin(x.z-va.x+1., d, .2);
    
    // spikes
    //vec4 w  = vor(1.*x-3.*rand(iNBeats*c.xx)-(.2+.1*iScale)*.1*valuenoise(x.xy-2.-1.*iTime));
