...
const char *gfx_frag =
 ...
 "float fdata(float v)"
 "{"
   "float x=mod(v,4.),y=v-x,s=mod(y,iFontWidth),f=(y-s)/iFontWidth;"
   "return x;"
 "}"
 ...
 "}"
 "float dist(vec2 x,vec2 y,vec2 v,vec2 i,float z)"
 "{"
   "return z=clamp(z,0.,1.),length(i-pow(1.-z,2.)*x-2.*(1.-z)*z*y-z*z*v);"
 "}"
 ...
 "}"
 "float circle(vec2 x,float y,float v)"
 "{"
   "return v-abs(length(x)-y);"
 "}"
 "float circleseg(vec2 x,float v,float y,float f,float z)"
 "{"
   "float i=clamp(sign(x.y)*acos(x.x/length(x)),f,z);"
   "vec2 l=v*vec2(cos(i),sin(i));"
 ...
 "{"
   "float i=min(zextrude(v.z,circle(v.xy-y*c.xy,y,f),x),zextrude(v.z,-line(v.xy,-y*c.yx,y*c.yx,f),x));"
   "return min(i,zextrude(v.z,circleseg(v.xy+y*c.xy,y,f,-pi/2.,pi/2.),x));"
 "}"
 "vec2 logo(vec3 v)"
 "{"
   "vec3 f=v;"
   ...
   "if(s>.5)"
     "z=rot(.5*pi*c.yyx)*z;"
   "y=vec2(pict210(z+.21*c.yyx,.5*x,.022,.005),s);"
   "i=mix(y,i,step(i.x,y.x));"
   ...
   "if(o<2.*d)"
             ...
             "{"
               "for(float b=-1.;b<=1.;b+=1.)"
                 "{"
                   ...
                   "y=vec2(length(max(abs(h)-u*c.xxx,0.)),s);"
                   "i=mix(y,i,step(i.x,y.x));"
                 "}"
             "}"
         "}"
     ...
     "}"
   "else"
     " y=vec2(o,1.),i=mix(i,y,step(y.x,i.x));"
   "return i;"
 "}"
 "vec2 tunnel(vec3 v)"
 "{"
 ...
 "{"
   ...
   "{"
     ...
     "for(int l=0;l<24;++l)"
       "x=min(x,dsg(d[2*l],d[2*l+1],s));"
     "for(int l=0;l<20;++l)"
       "x=min(x,dsp(a[3*l],a[3*l+1],a[3*l+2],s));"
   "}"
   "y=textbox(v,vec2(.485,.1),vec2(.6,.2),.5);"
   "z=vec4(x-.004,tc);"
   "y=add2(y,z);"
   "y=add2(y,vec4(abs(z.x)-.002,c.xxy));"
   "i=add2(i,y);"
   "f=max(f,step(y.x,1.5/iResolution.y)*blend(12.,25.));"
   "x=10.;"
   "{"
     ...
     "for(int l=0;l<17;++l)"
       "x=min(x,dsp(a[3*l],a[3*l+1],a[3*l+2],s));"
   "}"
   "y=textbox(v,vec2(.485,.03),vec2(.6,0.),.5);"
   "z=vec4(x-.004,tc);"
   "y=add2(y,z);"
   "y=add2(y,vec4(abs(z.x)-.002,c.xxy));"
   "i=add2(i,y);"
   "f=max(f,step(y.x,1.5/iResolution.y)*blend(13.,25.));"
   "x=10.;"
   "{"
     ...
     "for(int l=0;l<18;++l)"
       "x=min(x,dsp(a[3*l],a[3*l+1],a[3*l+2],s));"
   "}"
   "y=textbox(v,vec2(.435,-.02),vec2(.6,-.2),.5);"
   "z=vec4(x-.004,tc);"
   "y=add2(y,z);"
   "y=add2(y,vec4(abs(z.x)-.002,c.xxy));"
   "i=add2(i,y);"
   "f=max(f,step(y.x,1.5/iResolution.y)*blend(14.,25.));"
   "x=10.;"
   "{"
     ...
     "for(int l=0;l<24;++l)"
       "x=min(x,dsg(a[2*l],a[2*l+1],s));"
     "for(int l=0;l<25;++l)"
       "x=min(x,dsp(d[3*l],d[3*l+1],d[3*l+2],s));"
   "}"
   "y=textbox(v,vec2(.365,-.045),vec2(.6,-.4),.5);"
   "z=vec4(x-.004,tc);"
   "y=add2(y,z);"
   "y=add2(y,vec4(abs(z.x)-.002,c.xxy));"
   "i=add2(i,y);"
   "f=max(f,step(y.x,.01)*blend(15.,25.));"
   "return vec4(i.yzw,f)*smoothstep(1.5/iResolution.y,-1.5/iResolution.y,i.x);"
 "}"
 "vec3 raymarch(vec2 v,float x)"
 "{"
   "t=x;"
   "int y=200;"
   "float i=0.,f=50.,s=.0005;"
   "vec2 a;"
   "vec3 z=c.yzy,m=c.yyy,l=c.xyy,d=c.yyx,r=m+v.x*l+v.y*d,n=normalize(r-z),p,e;"
   "if(t<t_logo)"
     "f=20.,y=100;"
   "int o=0;"
   "for(int b=0;b<y;++b)"
     ...
     "{"
       ...
       "if(t<t_tunnel)"
         "{"
           "if(a.y<=1.)"
             "{"
               "vec3 g=c.yyx,w=normalize(reflect(-g,b)),h=normalize(p-z),u=abs(rot(.05*vec3(10.,21.,35.)*a.y+t+p.z)*c.xxy),R=abs(rot(.05*vec3(21.,35.,10.)*a.y+t+p.z)*c.yxx),k=abs(rot(.05*vec3(35.,10.,21.)*a.y+t+p.z)*c.xyx);"
           ...
           "else"
             " if(a.y<=2.)"
               "{"
                 "vec3 g=c.yyx,w=normalize(reflect(-g,b)),h=normalize(p-z),u=.1*c.xxx*length(abs(rot(.05*vec3(10.,21.,35.)*a.y+t+p.z)*c.xxy)),R=.1*c.xxx*length(abs(rot(.05*vec3(21.,35.,10.)*a.y+t+p.z)*c.yxx)),k=.2*c.xxx*length(abs(rot(.05*vec3(35.,10.,21.)*a.y+t+p.z)*c.xyx));"
                 "e=.1*u+dot(g,b)*.3*R+pow(abs(dot(w,h)),2.)*.9*k;"
                 "e=mix(e,.01*c.xxx,tanh(2.8*p.y));"
               "}"
             "else"
               " if(a.y<=3.)"
                 "{"
                   "vec3 g=normalize(c.yxx),w=normalize(reflect(-g,b)),h=normalize(p-z),u=abs(rot(.05*vec3(10.,21.,35.)*a.y+t+p.z)*c.xxy),R=abs(rot(.05*vec3(21.,35.,10.)*a.y+t+p.z)*c.yxx),k=abs(rot(.05*vec3(35.,10.,21.)*a.y+t+p.z)*c.xyx);"
 ...
 "{"
   ...
   "vec2 y=v/iResolution.yy-.5;"
   "const float i=.01;"
   "vec3 f=c.yyy;"
   "f=raymarch(y,iTime);"
   "f+=texture(iFont,y+.5).xyz;"
   "x=vec4(f,1.);"
 "}"
 "void main()"
 "{"
